...
public class AdjustStagePropAction extends ScriptedModule {
	public AdjustStagePropAction() {
		super("AdjustStagePropAction");
	}

	var Array defaultAlphaValues<String> = [];
	var Array props<String> = [];
	...
	private void onSongRetry(ScriptEvent event) {
		...
		if (props.size() > 0) {
			for (int i = 0; i < props.size(); i++) {
				PlayState.instance.currentStage.getNamedProp(props.get(i)).alpha = defaultAlphaValues.get(i);
			}
    ...
    public void onSongEvent(SongEventScriptEvent scriptEvent) {
		...
		if (scriptEvent.eventData.eventKind == "AdjustStageProp") {
			defaultAlphaValues.add(PlayState.instance.currentStage.getNamedProp(scriptEvent.eventData.value.propName).alpha);
			props.add(scriptEvent.eventData.value.propName);

			if (scriptEvent.eventData.value.duration != 0)
				FlxTween.tween(PlayState.instance.currentStage.getNamedProp(scriptEvent.eventData.value.propName), new Object() { public double alpha = scriptEvent.eventData.value.alpha; },
					PlayState.instance.currentStage.getNamedProp(scriptEvent.eventData.value.duration), {ease: FlxEase.quadOut});
			else
				PlayState.instance.currentStage.getNamedProp(scriptEvent.eventData.value.propName)
					.alpha = scriptEvent.eventData.value.alpha;
		}
	}

	public void onStateChangeEnd(StateChangeScriptEvent event) {
		props.clear();
	}
}